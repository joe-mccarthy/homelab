...
The role validates and pulls the Compose project, scales the known legacy
`ddns_cloudflare-ddns` Swarm service to zero, and starts the replacement. It
retires the legacy definition only after verification; a failed cutover stops
Compose and restores the legacy service. Unrelated Swarm resources are not
pruned.
